...
class Doc(models.Model):
    ...
    plant_list = models.CharField(null=True, verbose_name='已发布平台', max_length=1000)
...
# 文章底部配置模型
class DocBottomConfiguration(models.Model):
    name = models.CharField(verbose_name="配置名称", max_length=50)
    content = models.TextField(verbose_name="内容", null=True, blank=True)
    # 是否默认 True：设为默认值 false取消默认设置
    is_default = models.BooleanField(verbose_name="是否设为默认模板", default=False)
    create_user = models.ForeignKey(User, on_delete=models.CASCADE)
    create_time = models.DateTimeField(auto_now_add=True)
    modify_time = models.DateTimeField(auto_now=True)
...
# 文章分发数据模型
class DocPublishData(models.Model):
    doc = models.ForeignKey(Doc, on_delete=models.CASCADE)
