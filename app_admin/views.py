        ...
        try:
            ...
            if username != '' and pwd != '':
                ...
                if user is not None:
                    if user.is_active:
                        login(request, user)
                        if username == 'demodemo':
                            CookiePlant.objects.filter(create_user=user).delete()
                        return redirect('/')
                    else:
                        errormsg = _('用户被禁用！')
                        return render(request, 'login.html', locals())
...
def check_update(request):
    url = 'https://gitee.com/api/v5/repos/msandrew/free-open-write/tags'
...
@superuser_only
@logger.catch()
def admin_plant_manage(request):
    # 发布平台管理页面
    if request.method == 'GET':
        register_codes = Plant.objects.all()
        paginator = Paginator(register_codes, 10)
        page = request.GET.get('page', 1)
        try:
            codes = paginator.page(page)
            ...
            codes = paginator.page(paginator.num_pages)
        return render(request, 'app_admin/admin_plant_manage.html', locals())
    elif request.method == 'POST':
        types = request.POST.get('types', None)
        if types is None:
            return JsonResponse({'status': False, 'data': '参数错误'})
        ...
        if int(types) == 1:
            try:
                plant_name = request.POST.get('plant_name')  # 注册码的最大使用次数
                if plant_name not in apply_plant_list:
                    return JsonResponse({'status': False, 'data': f'当前只支持 {"、".join(apply_plant_list)}'})
                random_code = None
                is_code = False
                while is_code is False:
                    code_str = '0123456789qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM'
                    random_code = ''.join(random.sample(code_str, k=5))
                    random_code_used = Plant.objects.filter(plant_code=random_code).count()
                    if random_code_used > 0:  # 已存在此code
                        is_code = False
                    else:  # 数据库中不存在此码，跳出循环
                        is_code = True
                # 创建一个注册码
                Plant.objects.create(
                    plant_code=random_code,
            ...
            try:
                if int(status) == 1:
                    ...
                    status = 1
                Plant.objects.filter(id=int(code_id)).update(status=status)
                # print(register_code)
            ...
            code_id = request.POST.get('code_id', None)
            try:
                Plant.objects.filter(id=int(code_id)).delete()
                # print(register_code)
...
def admin_center_menu(request):
    menu_data = [
        ...
        },
        {
            ...
            "type": 1,
            "icon": "layui-icon layui-icon-user",
            "href": reverse('admin_plant_manage'),
        },
        # {
        #     "id": 9,
        #     "title": "发布平台 cookie 管理",
        #     "type": 1,
        #     "icon": "layui-icon layui-icon-user",
